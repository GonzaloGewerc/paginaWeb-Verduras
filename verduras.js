...
const PRODUCTS = [
  ...
  { id:"fit",      nombre:"Bolsón Frutal", precio:9990,  badge:"Peso: 5 kg",
    items:["Manzana","Banana","Mandarina","Naranja","Frutilla"], emoji:"",
    img:"imagen 3.webp" },
...
const renderPage = () => {
  ...
  $$(".js-elegir", productsGrid).forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      // Atajo: al abrir el modal, sumamos 1 a ese producto para acelerar la compra
...
const updateResumen = () => {
  ...
  if (!items.length) {
    ...
    syncSelectFromCart(); // aseguro validación coherente
    return;
  }
...
/* Mantenemos el <select multiple> sincronizado (solo para validación/retrocompat) */
const syncSelectFromCart = () => {
  ...
  bolson.dispatchEvent(new Event("change")); // para avisar a cualquier listener existente
};
...
const readRowQty = (row) => clampQty(parseInt($(".js-qty", row).value, 10));
...
const recalcPickerTotals = () => {
  // actualizar totales por fila (ya se actualizan on-change) y total general:
  pickerTotal.textContent = formatCurrency(getCartTotal());
};
...
pickerConfirm?.addEventListener("click", (e) => {
  e.preventDefault();
  // cerrar, sincronizar y actualizar resumen
  productPicker.close();
  updateResumen();
});
...
/* Reactivar scroll cuando cierre el modal */
productPicker?.addEventListener("close", () => {
  document.body.classList.remove("no-scroll");
});
productPicker?.addEventListener("cancel", () => {
  document.body.classList.remove("no-scroll");
});
...
/* Actualizar resumen también cuando cambie algo en el picker (por si queda abierto) */
productPicker?.addEventListener("close", updateResumen);
